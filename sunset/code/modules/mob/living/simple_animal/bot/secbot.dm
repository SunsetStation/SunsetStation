/mob/living/simple_animal/bot/secbot
	...
	declare_arrests = TRUE //When making an arrest, should it notify everyone on the security channel?
	idcheck = FALSE //If true, arrest people with no IDs
	weaponscheck = FALSE //If true, arrest people for weapons if they lack access
	check_records = TRUE //Does it check security records?
	arrest_type = FALSE //If true, don't handcuff
	var/hunting_icons = FALSE
	var/active_icons = FALSE

/mob/living/simple_animal/bot/secbot/beepsky
	name = "Officer Beep O'sky"
	desc = "It's Officer Beep O'sky! Powered by a potato and a shot of whiskey."
	icon_state = "secbot-sec"
	idcheck = FALSE
	weaponscheck = FALSE
	auto_patrol = TRUE
...
/mob/living/simple_animal/bot/secbot/pingsky
	...
	icon_state = "secbot-ai"
	radio_channel = RADIO_CHANNEL_AI_PRIVATE

/mob/living/simple_animal/bot/secbot/beepsky/prison
	name = "Officer Prisonsky"
	desc = "It's Officer Prisonsky! Delegated to labor camp duty after indiscriminately trying to arrest several 'innocent' assistants."
	icon_state = "secbot-prison"
	weaponscheck = TRUE

/mob/living/simple_animal/bot/secbot/beepsky/armory
	name = "Officer Seargant-at-Arms"
	desc = "It's Officer Seargant-at-Arms! Less likely to part with weapons than the Warden."
	icon_state = "secbot-armory"
	idcheck = TRUE
	weaponscheck = TRUE
	auto_patrol = FALSE
...
/mob/living/simple_animal/bot/secbot/update_icon()
	cut_overlays()
	if(mode == BOT_HUNT || mode == BOT_ARREST)
		if(active_icons)
			cut_overlay("secbot-active")
		add_overlay("secbot-hunt")
		add_overlay("secbot-baton")
		hunting_icons = TRUE
		active_icons = FALSE

	else if(mode == BOT_START_PATROL || mode == BOT_PATROL)
		if(hunting_icons)
			cut_overlay("secbot-baton")
			cut_overlay("secbot-hunt")
		add_overlay("secbot-active")
		hunting_icons = FALSE
		active_icons = TRUE
...
/mob/living/simple_animal/bot/secbot/retaliate(mob/living/carbon/human/H)
...
/mob/living/simple_animal/bot/secbot/judgement_criteria()
    var/final = FALSE
    if(idcheck)
...
/mob/living/simple_animal/bot/secbot/special_retaliate_after_attack(mob/user) //allows special actions to take place after being attacked.
	return
...
/mob/living/simple_animal/bot/secbot/cuff(mob/living/carbon/C)
	mode = BOT_ARREST
...
/mob/living/simple_animal/bot/secbot/attempt_handcuff(mob/living/carbon/C)
...
/mob/living/simple_animal/bot/secbot/stun_attack(mob/living/carbon/C)
	var/judgement_criteria = judgement_criteria()
...
/mob/living/simple_animal/bot/secbot/back_to_idle()
	anchored = FALSE
	mode = BOT_IDLE
	target = null
	last_found = world.time
...
/mob/living/simple_animal/bot/secbot/back_to_hunt()
	anchored = FALSE
	frustration = 0
	mode = BOT_HUNT
	INVOKE_ASYNC(src, .proc/handle_automated_action)
...
/mob/living/simple_animal/bot/secbot/look_for_perp()
	anchored = FALSE
	var/judgement_criteria = judgement_criteria()
...
/mob/living/simple_animal/bot/secbot/check_for_weapons(var/obj/item/slot_item)
...
/mob/living/simple_animal/bot/secbot/explode()
	...
	Sa.build_step = 1
	Sa.add_overlay("securitron_shell_hole")
