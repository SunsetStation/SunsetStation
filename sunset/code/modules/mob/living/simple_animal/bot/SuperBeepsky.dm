/mob/living/simple_animal/bot/secbot/grievous //This bot is powerful. If you managed to get 4 eswords somehow, you deserve this horror. Emag him for best results.
	name = "Securitron"
	desc = "A little security robot.  He looks less than thrilled. It has some odd grooves on it...?"
...
/mob/living/simple_animal/bot/secbot/grievous/toy //A toy version of general beepsky!
	...
	desc = "An adorable looking secbot. It has some odd grooves on it...?"
	health = 50
	maxHealth = 50
...
/mob/living/simple_animal/bot/secbot/grievous/update_icon()
	if(mode == BOT_HUNT)
		if(active_icons)
			cut_overlay("secbot-active")
		icon_state = "general-secbot-start"
		sleep(20)
		icon_state = "general-secbot-active"
		add_overlay("general-secbot-lights")
		add_overlay("general-secbot-swords")
		hunting_icons = TRUE
		active_icons = FALSE
		return
	else if(mode == BOT_START_PATROL || mode == BOT_PATROL)
		if(hunting_icons)
			cut_overlay("general-secbot-swords")
			cut_overlay("general-secbot-lights")
			icon_state = "general-secbot-end"
			sleep(20)
			icon_state = "secbot"
