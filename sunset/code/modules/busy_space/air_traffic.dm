...
/datum/lore/atc_controller/New()
	addtimer(CALLBACK(src, .proc/msg, "New shift beginning, resuming traffic control."), 30 SECONDS)
	next_message = world.time + rand(delay_min, delay_max)
	atc_process()

/datum/lore/atc_controller/proc/atc_process()
	...
	addtimer(CALLBACK(src, .proc/atc_process), 1 MINUTES)

/datum/lore/atc_controller/proc/msg(var/message,var/sender)
	ASSERT(message)
	var/obj/machinery/announcement_system/announcer = pick(GLOB.announcement_systems)
	announcer.radio.talk_into(announcer, "[message]", null, list(SPAN_ROBOT), announcer.get_default_language())
	//global_announcer.autosay("[message]", sender ? sender : "[GLOB.using_map.station_short] Space Control")
