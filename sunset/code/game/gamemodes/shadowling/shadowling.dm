...
/datum/species/shadow/ling/on_species_gain(mob/living/carbon/human/C)
	C.draw_sunset_parts()
	eyes_overlay = mutable_appearance('sunset/icons/mob/sling.dmi', "eyes", 25)
...
/mob/living/proc/add_sling()
	if(!istype(mind))
		return FALSE
	return mind.add_antag_datum(/datum/antagonist/shadowling)
...
/mob/living/proc/remove_sling()
	if(!istype(mind))
		return FALSE
	return mind.remove_antag_datum(/datum/antagonist/shadowling)
