...
/datum/sunset_ui/New(atom/n_datasource, n_width = 512, n_height = 512, n_assets = null)
	datasource = n_datasource
	window_id = REF(src)
	width = n_width
	height = n_height

/datum/sunset_ui/Destroy()
	close_all()
	if(isprocessing)
		STOP_PROCESSING(SSobj, src)
	return ..()

/datum/sunset_ui/process()
	if(auto_check_view)
		check_view_all()
	if(auto_refresh)
		render_all()

/datum/sunset_ui/proc/get_content(mob/target)
	return call(datasource, "oui_getcontent")(target)

/datum/sunset_ui/proc/can_view(mob/target)
	return call(datasource, "oui_canview")(target)

/datum/sunset_ui/proc/test_viewer(mob/target, updating)
...
/datum/sunset_ui/proc/render(mob/target, updating = FALSE)
...
/datum/sunset_ui/proc/render_all()
	for(var/viewer in viewers)
		render(viewer, TRUE)

/datum/sunset_ui/proc/close(mob/target)
	if(target && target.client)
		target << browse(null, "window=[window_id]")

/datum/sunset_ui/proc/close_all()
	for(var/viewer in viewers)
		close(viewer)
	viewers = list()

/datum/sunset_ui/proc/check_view_all()
	for(var/viewer in viewers)
		check_view(viewer)

/datum/sunset_ui/proc/check_view(mob/target)
	set waitfor = FALSE //Makes this an async call
	if(!test_viewer(target, TRUE))
		close(target)

/datum/sunset_ui/proc/call_js(mob/target, js_func, list/parameters = list())
	set waitfor = FALSE //Makes this an async call
	if(!test_viewer(target, TRUE))
		return
	target << output(list2params(parameters),"[window_id].browser:[js_func]")

/datum/sunset_ui/proc/call_js_all(js_func, list/parameters = list())
	for(var/viewer in viewers)
		call_js(viewer, js_func, parameters)

/datum/sunset_ui/proc/steal_focus(mob/target)
	set waitfor = FALSE //Makes this an async call
	winset(target, "[window_id]","focus=true")

/datum/sunset_ui/proc/steal_focus_all()
	for(var/viewer in viewers)
		steal_focus(viewer)

/datum/sunset_ui/proc/flash(mob/target, times = -1)
	set waitfor = FALSE //Makes this an async call
	winset(target, "[window_id]","flash=[times]")

/datum/sunset_ui/proc/flash_all(times = -1)
	for(var/viewer in viewers)
		flash(viewer, times)

/datum/sunset_ui/proc/href(mob/user, action, list/parameters = list())
	var/params_string = replacetext(list2params(parameters),"&",";")
	return "?src=[REF(src)];sui_action=[action];sui_user=[REF(user)];[params_string]"

/datum/sunset_ui/Topic(href, parameters)
