...
/datum/sunset_ui/themed/New(atom/n_datasource, n_width = 512, n_height = 512, n_content_root = "")
...
/datum/sunset_ui/themed/process()
	if(auto_check_view)
		check_view_all()
	if(auto_refresh)
		soft_update_fields()
...
/datum/sunset_ui/themed/proc/get_file(path)
...
/datum/sunset_ui/themed/proc/get_content_file(filename)
	return get_file("html/sunset_ui/content/[content_root]/[filename]")

/datum/sunset_ui/themed/proc/get_themed_file(filename)
	return get_file("html/sunset_ui/themes/[theme]/[filename]")

/datum/sunset_ui/themed/proc/process_template(template, variables)
...
/datum/sunset_ui/themed/proc/soft_update_fields()
	for(var/viewer in viewers)
		var/json = json_encode(call(datasource, "oui_data")(viewer))
		call_js(viewer, "updateFields", list(json))

/datum/sunset_ui/themed/proc/soft_update_all()
	for(var/viewer in viewers)
		call_js(viewer, "replaceContent", list(get_inner_content(viewer)))

/datum/sunset_ui/themed/proc/change_page(var/newpage)
	if(newpage == current_page)
		return
	current_page = newpage
	render_all()

/datum/sunset_ui/themed/proc/act(label, mob/user, action, list/parameters = list(), class = "", disabled = FALSE)
	if(disabled)
		return "<a class=\"disabled\">[label]</a>"
	else
		return "<a class=\"[class]\" href=\"" + href(user, action, parameters) + "\">[label]</a>"

/datum/sunset_ui/themed/nano
	theme = "nano"

/datum/sunset_ui/themed/paper
	theme = "paper"
