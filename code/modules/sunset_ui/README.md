...
This datum is a replacement for tgui which does not use any Node.js dependencies, and works entirely through raw HTML, JS and CSS. It's designed to be reasonably easy to port something from tgui to sunset_ui.

### How to create a UI

For this example, we're going to port the disposals bin from tgui to sunset_ui.

#### Step 1

In order to create a UI, you will first need to create an instance of `/datum/sunset_ui` or one of its subclasses, in this case `/datum/sunset_ui/themed/nano`.
...
/obj/machinery/disposal/bin/Initialize(mapload, obj/structure/disposalconstruct/make_from)
	. = ..()
	ui = new /datum/sunset_ui/themed/nano(src, 330, 190, "disposal_bin")
...
You will now need to make a template in `html/sunset_ui/content/{template_name}`.

Values defined as `@{value}` will get replaced at runtime by sunset_ui.

`html/sunset_ui/content/disposal_bin/index.html`
```html
<div class='display'>
	<section>
...
Now you need to implement the methods that provide data to sunset_ui. `oui_data` can be adapted from the `ui_data` proc that tgui uses.
...
You now need to hook in and ensure sunset_ui is invoked upon clicking. `render` should be used to open the UI for a user, typically on click.
...
Functions built into all objects to support sunset_ui. There are default implementations for most major superclasses.

##### `oui_canview(mob/user)`
Returns whether the specified user view the UI at this time.

##### `oui_getcontent(mob/user)`
Returns the raw HTML to be sent to the specified user. *This proc is not used in the themed subclass of sunset_ui.*

##### `oui_data(mob/user)`
Returns templating data for the specified user. *This proc is only used in the themed subclass of sunset_ui.*
